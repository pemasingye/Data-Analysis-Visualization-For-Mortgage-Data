...
WHERE MONTH >= 201511 AND MONTH <= 201610
GROUP BY MONTH
ORDER BY Mortgages_Per_Month ASC 
...
FROM [SQL Mortgages]..MTG_CUST$
WHERE Finadvisor = 'FA500_ON' AND DATEPART(yy, MONTH) = 2016 AND DATEPART(mm, MONTH) = 4
...
GROUP BY Region
ORDER BY Num_of_Mtg_Clients ASC
...
--Convert the Amount Value in Millions 

SELECT region, SUM(AMT_OF_LOAN)/1000000 AS 'Total Mortgage Value (in millions)'
FROM [SQL Mortgages]..MTG_CUST$ m
INNER JOIN [SQL Mortgages]..FinAdvMaster$ f 
ON m.Finadvisor = f.FinAdvID
INNER JOIN [SQL Mortgages]..Region$ r 
...
--To remove the number after the decimals 

SELECT region, CAST(ROUND(SUM(AMT_OF_LOAN)/1000000, 0) AS INT) AS 'Total Mortgage Value (in millions)'
FROM [SQL Mortgages]..MTG_CUST$ m
INNER JOIN [SQL Mortgages]..FinAdvMaster$ f
ON m.Finadvisor = f.FinAdvID
INNER JOIN [SQL Mortgages]..Region$ r
